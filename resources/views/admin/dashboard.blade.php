...
<div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm max-w-2xl">
    ...
    <div class="space-y-3">
        ...
        @if(($metrics['testimonials_pending'] ?? 0) > 0)
        <a href="{{ route('admin.testimonials.index') }}" class="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-50 transition border-l-2 border-amber-500">
            ...
            <div>
                <p class="text-sm font-semibold text-slate-900">Pending Testimonials</p>
                <p class="text-xs text-slate-500">{{ $metrics['testimonials_pending'] }} testimonials waiting for approval</p>
            </div>
        </a>
        @endif
