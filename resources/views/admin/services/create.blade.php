...
<div class="mb-6">
    ...
    <p class="text-sm text-slate-600 mt-1">Create a new service offering</p>
</div>

<form method="POST" action="{{ route('admin.services.store') }}" class="max-w-3xl">
    @csrf
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 space-y-6">
            ...
            <div class="grid grid-cols-4 md:grid-cols-8 gap-3">
                ...
                <label class="relative flex flex-col items-center gap-2 p-3 rounded-lg border-2 border-slate-200 cursor-pointer hover:border-blue-500 transition group">
                    <input 
                        ...
                        name="icon_type" 
                        value="{{ $type }}"
                        {{ old('icon_type', 'code') === $type ? 'checked' : '' }}
                        class="sr-only peer"
                        required
                    />
            ...
            <div class="grid grid-cols-4 md:grid-cols-8 gap-3">
                ...
                <label class="relative flex flex-col items-center gap-2 p-3 rounded-lg border-2 border-slate-200 cursor-pointer hover:border-{{ $color }}-500 transition">
                    <input 
                        ...
                        name="icon_color" 
                        value="{{ $color }}"
                        {{ old('icon_color', 'blue') === $color ? 'checked' : '' }}
                        class="sr-only peer"
                        required
                    />
        ...
        <div>
            ...
            <textarea 
                ...
                rows="6" 
                class="w-full rounded-lg border-slate-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                placeholder="Provide detailed information about this service, what it includes, benefits, etc."
                required
